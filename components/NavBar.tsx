...
const NavLink = ({ name, url }: Props): ReactElement => {
  return (
    <a
      href={url}
      className="block mt-2 lg:inline-block lg:mt-2 font-bold border-b-4 rounded border-transparent transition-all hover:border-white text-white mr-4 "
    >
      {name}
    </a>
  );
...
const NavButton = ({ name, url,className }: Props): ReactElement => {
  return (
    <a
      href={url}
      className={`inline-block text-sm px-4 py-2 leading-none border text-black font-bold rounded transition-colors mb-2 border-transparent lg:mt-0 ${className} `}
    >
      {name}
    </a>
  );
...
export default function NavBar(): ReactElement {
  ...
  const {menuState,authenticated} = useAppSelector((state:RootState) => state.navbar)
  const dispatch = useAppDispatch()

  return (
    <nav className="flex items-center justify-between flex-wrap bg-primary p-6">
      <div className=" hidden md:block font-semibold text-3xl text-white tracking-tight mr-6">
        <Link href="/">Quizlet</Link>
      </div>
        
      <div className=" flex flex-grow items-start md:items-center w-auto">
        ...
        <div className=" flex flex-col md:flex-row text-sm flex-grow">
        <button className="md:hidden" onClick={()=>dispatch(toggle())} >{menuIcon}</button>
          <div className={` ${menuState?"block":"hidden"} transition-all md:block`} >
          <NavLink name="Home" url="/" />
          <NavLink name="Subjects" url="/" />
          <NavLink name="Explanations" url="/" />
          <NavButton name="create" url="/create" className="bg-blue-400 hover:bg-blue-500 text-white" />
          </div>
        </div>
      </div>
    </nav>
